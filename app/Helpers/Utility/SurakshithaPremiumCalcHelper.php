 ...
 */
class SurakshithaPremiumCalcHelper
{
    ...
    public static $MAX_TERM = 30;
    public static $MAX_MATURITY_AGE = 70;
    
    private static $min_monthly_premium = 5000;
    private static $min_quartely_premium = 15000;
    private static $min_half_yearly_premium = 30000;
    private static $min_yearly_premium = 60000;
    ...
    public static function check_rider_validations($data){
        ...
        $bsa = $basic_mode_premium  * $policy_term  * 12 / self::get_mode_factor($frequency);
        ...
        if($frequency){
            ...
            foreach($data['member_main']['riders'] as $rider){
                ...
                if($rider['enabled'] === true && $rider['code'] == 'PA'){
                    ...
                    }else if( ((int)$rider['sum_assured'] > min(5*$bsa,20000000)) ){
                        return $rider['code'] .' Sum Assured cannot exceed '.number_format(min(5*$bsa,20000000));
                    }
                    else{
                ...
                if($rider['enabled'] === true && $rider['code'] == 'AFP'){
                    ...
                    if((int)$rider['sum_assured'] <=  0){
                        return $rider['code'] .' Sum Assured cannot be zero';
                    }else if( ((int)$rider['sum_assured'] > (5*$bsa)) ){
                        return $rider['code'] .' Sum Assured cannot exceed '.number_format((5*$bsa));
                    }
                    else{
                ...
                if($rider['enabled'] === true && $rider['code'] == 'CI'){
                    ...
                    if((int)$rider['sum_assured'] <=  0){
                        return $rider['code'] .' Sum Assured cannot be zero';
                    }else if( ((int)$rider['sum_assured'] > min($bsa,5000000)) ){
                        return $rider['code'] .' Sum Assured cannot exceed '.number_format(min($bsa,5000000));
                    }
                    else{
                ...
                if($rider['enabled'] === true && $rider['code'] == 'HB'){
                    $tmp_basic_annual_prem = $basic_mode_premium * 12 / self::get_mode_factor($frequency);
                    $hb_sum_assured = self::get_HB_sum_assured_by_plan_id((int)$rider['drp_val']);

                    if(($tmp_basic_annual_prem > 0 && $tmp_basic_annual_prem <= 24000 && $hb_sum_assured == 2000) ||
                        ($tmp_basic_annual_prem > 24000 && $tmp_basic_annual_prem <= 48000 && $hb_sum_assured == 4000) ||
                        ($tmp_basic_annual_prem > 48000 && $tmp_basic_annual_prem <= 60000 && $hb_sum_assured == 6000) ||
                        ($tmp_basic_annual_prem > 60000 && $tmp_basic_annual_prem <= 80000 && $hb_sum_assured == 8000) ||
                        ($tmp_basic_annual_prem > 80000 && $hb_sum_assured == 10000)){
                            $riders['HB_SA'] = 0;
                            $riders['HB'] = 1;
                            $riders['HB_OPTION']= (int)$rider['drp_val'];
                           
                    }else{
                        if($tmp_basic_annual_prem < 24001){
                            return 'Only 2,000 is allowed for the selected Basic Annual Premium for HB';
                        }else if($tmp_basic_annual_prem < 48001){
                            return 'Only 4,000 is allowed for the selected Basic Annual Premium for HB';
                        }else if($tmp_basic_annual_prem < 60001){
                            return 'Only 6,000 is allowed for the selected Basic Annual Premium for HB';
                        }else if($tmp_basic_annual_prem < 80001){
                            return 'Only 8,000 is allowed for the selected Basic Annual Premium for HB';
                        }else if($tmp_basic_annual_prem > 10000){
                            return 'Only 10,000 is allowed for the selected Basic Annual Premium for HB';
                        }
                    }
                }
            ...
            if($has_spouse){
                ...
                foreach($data['member_spouse']['riders'] as $rider){
                    ...
                    if($rider['enabled'] === true && $rider['code'] == 'CI-SPOUSE'){
                        ...
                        if((int)$rider['sum_assured'] <=  0){
                            return $rider['code'] .' Sum Assured cannot be zero';
                        }else if( ((int)$rider['sum_assured'] > min(($bsa * 0.75),5000000,$riders['CI_SA'])) ){
                            return $rider['code'] .' Sum Assured cannot exceed '.number_format(min(($bsa * 0.75),5000000,$riders['CI_SA']));
                        }
                        else{
                    ...
                    */
                    if($rider['enabled'] === true && $rider['code'] == 'FR-SPOUSE'){
                        ...
                        if((int)$rider['sum_assured'] <=  0){
                            return $rider['code'] .' Sum Assured cannot be zero';
                        }else if( ((int)$rider['sum_assured'] > min(($bsa * 0.75),1000000)) ){
                            return $rider['code'] .' Sum Assured cannot exceed '.number_format(min(($bsa * 0.75),1000000));
                        }
                        else{
            ...
            if( $child_count > 0){
                for($i=0; $i < $child_count ; $i++ ){
                    ...
                    if( ($ch_age_nearest + $policy_term) > 18){
                        $ch_rider_term = ($policy_term - ($ch_age_nearest + $policy_term -18));
                    }
                    $ch_gender =  ($data['member_child'][$i]['title']=='Master'?'Male':'Female');
                    

                    //FR-CHILD - If rider enabled
                    
                    if($data['child_riders'][0]['enabled'] === true){
                        $riders['FR-CHILD'] = 1;

                        if($riders['FR-SPOUSE'] == 0 ){
                            return 'FR-SPOUSE cover not selected';
                        }
                        for($j=0; $j< $child_count; $j++){
                            //If rider enabled
                            if($data['child_riders'][0]['child'][$j]['checked'] === true && $j==$i){

                                if($ch_age_exact_years > 18 ){
            ...
            if(($riders['FR-CHILD'] == 1 ) && ($child_count != $verify_child_count)){
    ...
    public static function get_FR_CHILD_sum_assured_by_plan_id($plan_id){
        $val = 0;
        switch ($plan_id) {
            case '1':
                $val = 25000;
    ...
    public static function get_hb_sum_assured_if_plan_selected($data){
        foreach($data['member_main']['riders'] as $rider){

            if($rider['enabled'] === true && $rider['code'] == 'HB'){
                return  self::get_HB_sum_assured_by_plan_id((int)$rider['drp_val']);
            }
        }
        return 0;
    }
