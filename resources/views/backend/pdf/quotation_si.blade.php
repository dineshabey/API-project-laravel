...
<head>
    ...
    <style type="text/css">
        ...
        .si-lang-text-pdf
        {
            font-family: 'malithiunicode', "freesans" !important;
        }
        th {
        ...
        body {
            font-family: 'malithiunicode', "freesans";
...
<div class="wrapper text-center" style="margin-bottom: 10px;">
    <table style="width:100%">
        <tr>
            <td width="50%"><h2><span class="en-lang-text-pdf">{{ __('quo_pdf.backend.title_main_hnb') }}</span> {{ __('quo_pdf.backend.title_main') }} </h2></td>
            <td width="50%" class="text-right"><strong class="en-lang-text-pdf">Quotation No: {{ $quotationData['quote_no'] }}
...
$RESTRICTED = '**'; // Restricted rider indicator
        ...
        foreach ($prem_data['member_main'] as $rider) {
            if ($rider['rider_code'] == 'AD-LIFE') {
                ...
                $TPD_OPT = ($rider['option'] == '1' ? '(අනතුරු සහ අසනීප)' : '(අනතුරකදී)');
                $rider_prem_total += $rider['total_premium'];
                ...
                $rider_prem_total += $rider['total_premium'];
                $MIB_2_OPT = ($rider['option'] == '1' ? '(මරණයකදී)' : '(මරණයකදී හෝ පූර්ණ නිත්‍ය අකර්මන්‍යතාවයකදී)');
        ...
        ?>
                    <table class="bordered width-100pc">
                        ...
                        <tr>
                            <td colspan="2">{{ __('quo_pdf.backend.tb4.rider_main.tot_permanent_disability_benift') }} <?php echo $TPD_OPT; ?></td>
                        ...
                        <tr>
                            <td colspan="2">{{ __('quo_pdf.backend.tb4.rider_main.permanent_partial_disability_benifit') }}</td>
...
<div class="wrapper">
    <div class="divTable">
        ...
        echo " සමග " . number_format((double) $AD_LIFE_SA);
    }
    ?>
                            </td>
    <?php if ((double) $SP_AD_LIFE_SA != 'N/A') {?>
        ...
        echo " සමග " . number_format((double) $AD_LIFE_SA + (double) $ACC_DTH_SA);
    }
    ?>
                            </td>
                            <?php if ((double) $SP_AD_LIFE_SA != 'N/A') {?>
                ...
                <div class="divTableCell">
                    <table class="bordered width-100pc inter_tbl_spacing">
                        ...
                        <tr>
                            <td>
                                <strong>
                            රක්ෂණයේ පළමු වසර {{ $quotationData['premium_term'] }} තුළ ගෙවිය යුතු {{ $mode_list[$str_mode]}} වාරිකය (රු)
                                </strong></td>
                            <td class="text-right">
...
if ($quotationData['premium_term'] < $quotationData['policy_term'] && ($prem_data['summary']['frequency'] != 'S')) {
        ?>
                        <tr>
                            <td><strong>
                            රක්ෂණයේ වසර {{ $quotationData['premium_term'] }} සම්පූර්ණ වූ පසු, වසර  {{ $prem_data['summary']['policy_term'] }} තෙක් ගෙවිය යුතු {{ $mode_list[$str_mode]}} වාරිකය (රු)
                            </strong></td>
                            <td class="text-right">
        ...
        <div class="divTableBody">
            <div class="divTableRow">
                <div class="divTableCell">
                    <table class="bordered width-100pc inter_tbl_spacing ">
                        <tr>
                            <th colspan="4">කල්පිරීමේදී ඔබේ අරමුදලේ වටිනාකම  - {{ $mode_list[$str_mode]}} වාරික ගෙවීමේ ආකාරය අනුව </th>
                        </tr>
                        <tr>
                            <th class="text-center">
...
<htmlpagefooter name="page-footer">
    <div class="page_num en-lang-text-pdf">Page {PAGENO}</div>
</htmlpagefooter>
</body>
</body>
</html>